...
//objeto vacio para guardar el conteo de cuantas veces aparece cada sonido
const soundCount = [];

//recorremos la lista de usuarios
for (const user of users) {
  //entramos en los sonidos favoritos de los usuarios
  for (const sound in user.favoritesSounds) {
    //contamos cada sonido
    if (soundCount[sound]) {
      //si el sonido ya existe en el objeto sumamos 1
      soundCount[sound]++;
    } else {
      soundCount[sound] = 1; //si no existe lo creamos con valor 1
    }
  }
}
